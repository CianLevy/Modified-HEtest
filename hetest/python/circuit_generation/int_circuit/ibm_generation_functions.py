...
gate_types = ['LADD', 'LADDconst', 'LMUL', 'LMULconst']
...
def make_random_two_inp_gate(B, L, ultimate_level, penultimate_level, gate_name,
...
def make_random_one_inp_and_const_inp_gate(B, L, ultimate_level, penultimate_level,
    ...
    input1_index = sr.randint(0, len(ultimate_level) - 1)
    input1 = ultimate_level[input1_index]
    const = ci.Input([ib.IBMBatch([int(sr.randint(0,B))
                                   for inp_ind in xrange(L)])])
    return gate_factory(gate_name, input1, const, circuit)

def make_random_one_inp_and_const_int_gate(B, L, ultimate_level, penultimate_level,
                                           gate_name, circuit, gate_factory):
...
def make_random_two_inp_and_const_inp_gate(B, L, ultimate_level, penultimate_level,
                                           gate_name, circuit, gate_factory):
...
def make_random_gate(B, L, ultimate_level, penultimate_level, gate_name, circuit):
...
def make_random_input(L, W, B):
    ...
    return ci.Input([ib.IBMBatch([int(sr.randint(0,B))
