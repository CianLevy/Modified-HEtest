...
#include <iostream>
#include "integer-ctxt-container.h"
/*
class test{
public:
 test operator + (test const &obj) const {test t; return t; }
  test operator & (test const &obj) const {test t; return t; }
   test operator * (test const &obj) const {test t; return t; }
      test operator ^ (test const &obj) const {test t; return t; }
      test operator ~ () const {test t; return t; }
      test operator | (test const &obj) const {test t; return t; }
     // test operator << (std::ostream& os, test const &obj) const {test t; return "null"; }
   test(){ std::cout << "Constructed type\n";}

   friend std::ostream& operator << (std::ostream &out, const test &c);
};

std::ostream& operator << (std::ostream &out, const test &c){ return out;} */
...
class Gate {
 ...
 public:
  ...
  // is called.
  virtual CtxtContainer Evaluate() = 0;
 ...
 protected:
  ...
  //const CtxtContainer& output() const{ return output_; }
  CtxtContainer  output() {return output_;};
  void SetOutput(CtxtContainer output) {
    output_ = output;
  }
 
 private:
  CtxtContainer output_;
